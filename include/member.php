...
class User{	
	...
	function find_all_user($name=""){
	 ...
	 static function AuthenticateUser($email="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblemployee` WHERE `USERNAME`='" . $email . "' and `PASSWRD`='" . $h_upass ."' AND EMPSTATUS!='Attrited' LIMIT 1");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
		 $found_user = $mydb->loadSingleResult();
			...
			$_SESSION['IDENTIFICATION']    = $found_user->IDENTIFICATION;
            $_SESSION['EMPSTATUS'] 	= $found_user->EMPSTATUS;
			...
			$_SESSION['EMPROLE']       = $found_user->EMPROLE;
			$_SESSION['STRDATE']       = $found_user->STRDATE;
            return true;
			}else{
				return false;
			}	
